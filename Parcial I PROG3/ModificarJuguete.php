...
class Manejadora{
    public static function Modificar($tipo, $precio, $paisOrigen, $path, $tipoA, $precioA, $paisOrigenA, $pathA){
        $obj = new stdClass();
        $destino = "";
        $juguete = new Juguete($tipoA, $precioA, $paisOrigenA, "./juguetes/imagenes/" . $pathA);
        if($path != ""){
            $destino = "./juguetes/imagenes/" . $tipo . "." . $paisOrigen . "." . date("Gis") . "." . "jpg";
        }
        if($juguete->Modificar($tipo, $precio, $paisOrigen, $destino)){
            $destinoModificado = "./juguetes_Modificados/" . $juguete->getTipo() . "." .  $juguete->getPais() . "." . "modificado." . date("Gis") . "." . "jpg";
            copy($juguete->getPath(), $destinoModificado);
            unlink($juguete->getPath());
            if($path != ""){
                if (move_uploaded_file($_FILES["archivo"]["tmp_name"], $destino)){
                    header("Location:./Listado.php");
                } else{
                    $obj->exito = false;
                    $obj->mensaje = "No se pudo modificar el juguete";
                }
            } else{
                header("Location:./Listado.php");
            }
            ...
            $obj->exito = false;
            $obj->mensaje = "No existe el juguete";
        }
        return $obj;
    }
}
...
$paisOrigenA = isset($_POST["paisA"]) ? $_POST["paisA"] : "";
